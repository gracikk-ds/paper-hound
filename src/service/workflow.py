...
class WorkflowService:
    ...
    def process_daily_cycle(self, date: datetime.date | None = None) -> None:
        ...
        logger.info(f"Starting daily cycle for {date_str}")
        ...
        try:
            ...
            candidates = self.processor.search_papers(query=query, k=10, start_date_str=date_str, end_date_str=date_str)
        except Exception as exp:  # noqa: BLE001
        ...
        for paper in candidates:
            try:
                is_relevant = self.classifier.classify(title=paper.title, summary=paper.summary)

                if is_relevant:
                    logger.info(f"Paper '{paper.title}' classified as relevant. Processing...")
                    url = self.process_paper_summary_and_upload(paper_id=paper.paper_id, category="Image Editing")
                    if url:
